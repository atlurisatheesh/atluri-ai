...
def get_backend_pid(base_http: str, token: str) -> int:
...
async def test_single_session(base_ws: str, token: str) -> dict:
...
async def _session_worker(base_ws: str, token: str, marker: str, transcript_lines: list[str], result: dict):
    room_id = str(uuid.uuid4())
    ws_url = f"{base_ws}?room_id={room_id}&role=devops&participant=candidate&token={token}"
    local_recv = []
    try:
...
async def test_two_parallel(base_ws: str, token: str) -> dict:
    ra, rb = {}, {}
    await asyncio.gather(
        _session_worker(base_ws, token, "SESSION_A", ["line1", "line2", "line3"], ra),
        _session_worker(base_ws, token, "SESSION_B", ["line1", "line2", "line3"], rb),
    )
    ...
    def extract_texts(events):
        ...
        for e in events:
            kind = e.get("type")
            if kind in {"transcript", "partial_transcript", "interviewer_question"}:
    ...
    leakage = any("SESSION_B" in text for text in a_texts) or any("SESSION_A" in text for text in b_texts)
...
async def test_kill_mid_llm(base_ws: str, token: str) -> dict:
    room_id = str(uuid.uuid4())
...
async def test_flood_websocket(base_ws: str, token: str) -> dict:
    room_id = str(uuid.uuid4())
...
async def test_10_concurrent(base_ws: str, token: str, backend_pid: int, stagger_seconds: float = 0.0) -> dict:
...
async def run_all(port: int, stagger_seconds: float = 0.0, backend_pid: int | None = None) -> dict:
    base_http = f"http://127.0.0.1:{port}"
    base_ws = f"ws://127.0.0.1:{port}/ws/voice"
    token = build_dev_token()
    effective_pid = backend_pid if backend_pid is not None else get_backend_pid(base_http, token)
    ...
    await run_guard("test1_single_session", lambda: test_single_session(base_ws, token))
    await run_guard("test2_two_parallel", lambda: test_two_parallel(base_ws, token))
    await run_guard("test3_kill_mid_llm", lambda: test_kill_mid_llm(base_ws, token))
    await run_guard("test4_flood", lambda: test_flood_websocket(base_ws, token))
    await run_guard("test5_10_concurrent", lambda: test_10_concurrent(base_ws, token, effective_pid, stagger_seconds=stagger_seconds))

    return results


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run runtime stress suite")
    ...
    results = asyncio.run(run_all(port=args.port, stagger_seconds=args.stagger_seconds, backend_pid=args.backend_pid))
