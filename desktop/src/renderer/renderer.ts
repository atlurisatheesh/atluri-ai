...
(function () {
  ...
  settingsSaveBtn?.addEventListener("click", async () => {
    ...
    const sessionSetup = {
      ...
      procedures: (document.getElementById("setupProcedures") as HTMLTextAreaElement)?.value || "",
      priorityQuestions: (document.getElementById("setupPriorityQuestions") as HTMLTextAreaElement)?.value || "",
      ...
      codingLanguage: (document.getElementById("setupCodingLang") as HTMLSelectElement)?.value || "",
      ...
      model: (window as any).getSelectedModel?.() || "general",
      coachStyle: (document.getElementById("coachStyle") as HTMLSelectElement)?.value || "balanced",
      coachIndustry: (document.getElementById("coachIndustry") as HTMLSelectElement)?.value || "default",
  ...
  async function restoreSettings() {
    ...
    if (ss) {
      ...
      if (ss.model) {
        ...
        (window as any).__pendingModel = normalizedModel;
        const modelEl = document.querySelector(`[data-model="${normalizedModel}"]`) as HTMLElement;
        if (modelEl) {
...
(function initNextGenUI() {
    ...
    tab.addEventListener("click", () => {
      ...
      if (mode === "coding") {
        ...
        if (setupStep1 && !setupStep1.classList.contains("open")) {
          (window as any).toggleSetupStep(1);
        }
      }
  ...
  function updateStepNumbers() {
    ...
    const step3Done = selectedModel !== "general" || !coachEnabled || ($("coachStyle") as HTMLSelectElement)?.value !== "balanced";
  ...
  // ═══════════════════════════════════════════════════════════
  const SCENARIO_CATALOG: { category: string; scenarios: { value: string; label: string }[] }[] = [
  ...
  function populateScenarios() {
    const sel = $("setupScenario") as HTMLSelectElement;
    if (!sel) return;
  ...
  (window as any).selectModelCard = function (el: HTMLElement) {
    ...
    if (tierNameEl && def) tierNameEl.textContent = def.name;
    updateStepNumbers();
    console.log(`[model] selected: ${model}`);
  };

  (window as any).toggleModelTier = function () {
  ...
  // CUSTOM INDUSTRY
  // ═══════════════════════════════════════════════════════════
  (window as any).addCustomIndustry = function () {
    const name = prompt("Enter custom industry name:");
    if (!name || !name.trim()) return;
    const sel = $("coachIndustry") as HTMLSelectElement;
    if (!sel) return;
  ...
  // ═══════════════════════════════════════════════════════════
  const QA_PREP_QUESTIONS: { category: string; questions: string[] }[] = [
  ...
  let qaPrepAnswers: Record<string, string> = {};
  ...
  function renderQAPrepModal() {
    ...
    if (countEl) countEl.textContent = `${totalAnswered} answer${totalAnswered !== 1 ? 's' : ''} prepared`;
  }

  (window as any).openQAPrep = function () {
    ...
    try {
      ...
      if (saved) qaPrepAnswers = JSON.parse(saved);
    } catch { /* ignore */ }
    renderQAPrepModal();
  ...
  (window as any).closeQAPrep = function () {
    const modal = $("qaPrepModal");
    if (modal) modal.style.display = "none";
    // Persist answers
    try { localStorage.setItem("phantomveil_qa_answers", JSON.stringify(qaPrepAnswers)); } catch { /* ignore */ }
  };

  (window as any).toggleQAQuestion = function (el: HTMLElement) {
    // Don't toggle if click was on textarea
    if ((event?.target as HTMLElement)?.tagName === "TEXTAREA") return;
    el.classList.toggle("expanded");
  };

  (window as any).saveQAAnswer = function (textarea: HTMLTextAreaElement) {
    const key = textarea.dataset.qaKey;
    if (!key) return;
    qaPrepAnswers[key] = textarea.value;
    const questionEl = textarea.closest(".qa-question");
    if (questionEl) questionEl.classList.toggle("has-answer", !!textarea.value.trim());
    // Update count
    const total = Object.values(qaPrepAnswers).filter(v => v.trim()).length;
    const countEl = $("qaPrepCount");
    if (countEl) countEl.textContent = `${total} answer${total !== 1 ? 's' : ''} prepared`;
  };
  ...
  function getSessionContext(): Record<string, string> {
    return {
      ...
      procedures: ($("setupProcedures") as HTMLTextAreaElement)?.value || "",
      priorityQuestions: ($("setupPriorityQuestions") as HTMLTextAreaElement)?.value || "",
      industry: ($("setupIndustry") as HTMLSelectElement)?.value || "default",
      experience: ($("setupExperience") as HTMLSelectElement)?.value || "mid",
      codingLanguage: ($("setupCodingLang") as HTMLSelectElement)?.value || "",
      ...
      coachIndustry: ($("coachIndustry") as HTMLSelectElement)?.value || "default",
  ...
  const companyInput = $("setupCompany") as HTMLInputElement;
  if (companyInput) {
    companyInput.addEventListener("blur", () => {
      ...
      const procEl = $("setupProcedures") as HTMLTextAreaElement;
      const prioEl = $("setupPriorityQuestions") as HTMLTextAreaElement;
  ...
  // ═══════════════════════════════════════════════════════════
  function updateStealthPanel(health: any) {
    ...
    if (score < 50) ring.classList.add("danger");
  ...
  async function pollStealthHealth() {
    try {
      const dApi = (window as any).atluriinDesktop;
      if (dApi?.getStealthHealth) {
        const health = await dApi.getStealthHealth();
        updateStealthPanel(health);
      } else {
        // Fallback: fetch from backend
        const base = ($("backendUrl") as HTMLInputElement)?.value || "http://localhost:9010";
  ...
  // ═══════════════════════════════════════════════════════════
  const opacitySlider = $("opacitySlider") as HTMLInputElement;
  if (opacitySlider) {
  ...
  function getPresets(): { name: string; data: Record<string, string>; ts: number }[] {
    try {
  ...
  function savePresets(presets: { name: string; data: Record<string, string>; ts: number }[]) {
    localStorage.setItem(PRESET_KEY, JSON.stringify(presets.slice(0, 20)));
  }

  (window as any).savePreset = function () {
  ...
  (window as any).togglePresetList = function () {
    ...
    const closeHandler = (e: Event) => {
      if (!(e.target as HTMLElement).closest(".preset-bar")) {
        dropdown.classList.remove("open");
  ...
  (window as any).loadPreset = function (index: number) {
    const presets = getPresets();
    ...
    const presetSearch = $("presetSearch") as HTMLInputElement;
    if (presetSearch) presetSearch.value = preset.name;
    const dropdown = $("presetDropdown");
  ...
  // ═══════════════════════════════════════════════════════════
  (window as any).quickRun = function () {
