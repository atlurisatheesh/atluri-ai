  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {items.map((item, index) => (
        <div key={index} className="rounded-xl border border-white/[0.08] overflow-hidden bg-white/[0.02]">
          <button
          ...
          <AnimatePresence>
            {openIndex === index && (
              ...
              >
                <div className="px-6 pb-4 text-textMuted leading-relaxed">{item.content}</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
