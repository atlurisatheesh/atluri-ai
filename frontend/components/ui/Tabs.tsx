...
export default function Tabs({ tabs, className = "" }: TabsProps) {
  ...
  return (
    <div className={className}>
        ...
        {tabs.map((tab, i) => (
          <button
            ...
            className={`relative px-5 py-3 text-sm font-semibold whitespace-nowrap transition-colors cursor-pointer ${
              active === i ? "text-brand-cyan" : "text-textMuted hover:text-textPrimary"
            }`}
            onClick={() => setActive(i)}
          >
      ...
      <motion.div key={active} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="pt-6">
        {tabs[active].content}
      </motion.div>
    </div>
  );
